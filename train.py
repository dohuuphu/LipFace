...
from losses import CombinedMarginLoss, cos_linearity_lipschitz
...
def main(args):
    ...
    for epoch in range(start_epoch, cfg.num_epoch):
        ...
        for _, (img, local_labels) in enumerate(train_loader):
            ...
            # print('local_embeddings====', len(local_embeddings))
            # print('local_embeddings====', local_embeddings)

            local_embeddings = torch.reshape(local_embeddings, (cfg.batch_size, (1+cfg.num_img_lip), cfg.embedding_size)) ##
            loss_arc: torch.Tensor = module_partial_fc(local_embeddings[:, 0, :].contiguous(), local_labels, opt) ##
            
            # cos_linearity
            img = torch.reshape(img, (cfg.batch_size, 1+cfg.num_img_lip, 3, 112, 112)) ##
            loss_lip = cos_linearity_lipschitz(img, local_embeddings, cfg.lip, cfg.squared, cfg.detach_HR, cfg.p) ##
            
            loss = loss_arc + cfg.lamb_lip * loss_lip
