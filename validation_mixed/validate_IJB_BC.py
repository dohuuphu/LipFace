...
def identification(data_root, dataset_name, img_input_feats, save_path):
    ...
    identification_result, is_correct, similarity_score_gt, rank = eval_helper_identification.evaluation(probe_feats, gallery_feats, mask)
    pd.DataFrame(identification_result, index=['identification']).to_csv(os.path.join(save_path, "identification_result.csv"))
    pd.DataFrame(is_correct).to_csv(os.path.join(save_path, "is_correct.csv"))
    pd.DataFrame(similarity_score_gt).to_csv(os.path.join(save_path, "similarity_score_gt.csv"))
    pd.DataFrame(rank).to_csv(os.path.join(save_path, "rank.csv"))
...
def verification(data_root, dataset_name, img_input_feats, save_path):
    ...
    score, pairs = eval_helper_verification.verification(template_norm_feats, unique_templates, p1, p2)

    # # Step 5: Get ROC Curves and TPR@FPR Table
    score_save_file = os.path.join(save_path, "verification_score.npy")
    np.save(score_save_file, score)
    result_files = [score_save_file]
    eval_helper_verification.write_result(result_files, save_path, dataset_name, label, pairs)
...
def load_pretrained_model(model_name='ir50'):
    ...
    return model
    '''
    model = get_model(adaface_models[model_name][1], dropout=0.0, fp16=False, num_features=512).cuda()
    pretrained_dict = torch.load(adaface_models[model_name][0])
...
if __name__ == '__main__':
    ...
    adaface_models = {
        ...
        'ir101_ms1mv3': ["/mnt/HDD1/yuwei/insightface_lipface/work_dirs/ms1mv3_r100_lip_adaptive/model.pt", 'r100'],
        'lipface_r100_ms1mv3': ["/mnt/HDD1/yuwei/insightface_lipface/work_dirs/ms1mv3_r100_lip_adaptive/model.pt", 'r100'],
        'ir101_webface4m': ["/mnt/HDD4/yuwei/Lipface/AdaFace/pretrained/adaface_ir101_webface4m.ckpt", 'ir_101'],
